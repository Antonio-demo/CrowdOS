...
public class Sensor_MessageController {
    ...
    private static final Logger logger =
            (Logger) LoggerFactory.getLogger(Sensor_DetailController.class);
    ...
    ///2019.7.25 Upload the text message and picture of the task, two parameters,
    // user_task text message + single picture
    ...
    public ResponseEntity<Familiar_Sensor> add_Sensor_File(@RequestPart("familiar_sensor") Familiar_Sensor familiar_sensor, @RequestPart("file") MultipartFile file) throws Exception {
        ...
        //sm4算法加解密过程
        String encryptStr = SM4.encryptEcb("cdfexlskjglslldks581",fileByte);
        System.out.println("加密之后的数据流："+encryptStr);
        String decryptStr = SM4.decryptEcb("cdfexlskjglslldks581",encryptStr);
        System.out.println("解密之后的数据流："+decryptStr);
        //sm4验证过程
        if(SM4.verifyEcb("cdfexlskjglslldks581",encryptStr,decryptStr)){
            System.out.println("ok");
        }

        //2019.9.17  Read the size of the uploaded file,
        // cannot exceed the maximum supported by SSM  ,unit:B
    ...
    @ResponseBody
    // User class           File type parameters (can be files, videos, pictures)
    public ResponseEntity<Sensor_Message> add_Sensor_File_Bytes(@RequestPart("sensor_message") Sensor_Message sensor_message, @RequestPart("file") MultipartFile file) throws Exception {
...
//        System.out.println("转换成功的二进制文件类型："+ getType(file.getBytes()));
        ...
        String encryptfilebyte = SM4.encryptEcb("cdfexlskjglslldks571",fileByte);
        System.out.println("加密之后的数据流："+encryptfilebyte);
        String decryptfilebyte = SM4.decryptEcb("cdfexlskjglslldks571",encryptfilebyte);
        System.out.println("解密之后的数据流："+decryptfilebyte);
        System.out.println("decryptfilebyte="+decryptfilebyte);
...
//        new BufferedReader(new InputStreamReader(inputStream, StandardCharsets.UTF_8)).lines().forEach(this::handleLine);
        ...
        Integer MaxId = sensor_messageService.getMaxId().getSensor_messageId();
        //用SM4加密当前表格的主键ID
        String encryptID = SM4.encryptEcb("cdfexlskjglslldks581",Integer.toString(MaxId));
        System.out.println("加密之后的数据流："+encryptID);
        String decryptID = SM4.decryptEcb("cdfexlskjglslldks581",encryptID);
        Integer ID = Integer.valueOf(decryptID).intValue();
        System.out.println("解密之后的数据流："+decryptID);
        System.out.println("MacId="+decryptID);
...
//                    System.out.print(i +"-"+ s.charAt(i)+"  ");
                ...
                ID++;
                sensor_bytetemp.setSensor_messageId(ID);
